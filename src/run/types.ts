 ...
 *   - The run was misconfigured          -> neither is at fault     -> never heal
 *
...
export const FAILURE_KINDS = [
  ...
  /**
   * The test could not be run as written — a `valueRef` naming an environment
   * variable that is not set, most commonly.
   *
   * Given its own kind because the alternative is worse than untidy. Resolving a
   * value happens before the element is touched, so the error escaped the step
   * and surfaced as a run-level fault with every step skipped: "the application
   * was never reachable". The application was fine. Reporting an operator
   * mistake as an outage sends someone to look at the wrong system, which is the
   * precise failure mode this classifier exists to prevent.
   */
  'CONFIGURATION_ERROR',
