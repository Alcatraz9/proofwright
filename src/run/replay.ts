...
export type RunEvent =
  ...
  /**
   * Emitted before a step is attempted. Nothing in the CLI needs this — its
   * output is a transcript after the fact — but a live view has to be able to
   * show that a step is in progress. Without it the first sign of a step is its
   * own completion, so a slow step looks like a stalled run.
   */
  | { type: 'stepStarting'; stepId: string; action: Action; index: number; total: number }
  ...
  /** A post-step inspection failed. Never the step's verdict — the step passed. */
  | { type: 'inspectionError'; stepId: string; message: string };
...
export interface ReplayOptions {
  ...
  /**
   * Called after a step has passed and the page has settled.
   *
   * Injected rather than imported for the same reason healing is: the replay must
   * stay usable, and testable, with no visual, accessibility or security
   * machinery anywhere near it. Only invoked after a pass — inspecting the page a
   * failing step left behind would report findings about a state the test never
   * meant to reach.
   */
  onStepSettled?: (params: { page: Page; step: BaselineStep; monitor: HealthMonitor }) => Promise<void>;
}
...
export async function runSteps({
  ...
  for (const [index, step] of steps.entries()) {
    ...
    if (result.status === 'passed' && onStepSettled) {
      // An inspection that throws must not become the step's verdict. The step
      // already passed against the application; a broken analyser is our problem,
      // not the app's, and reporting it as a test failure would be a lie.
      try {
        await onStepSettled({ page: context.page, step, monitor: context.monitor });
      } catch (err) {
        onEvent?.({
...
async function runStep(
  ...
  try {
    value = step.valueRef ? resolveValueRef(step.valueRef) : step.value;
  } catch (err) {
    // Attributed to this step rather than allowed to escape. Unhandled, it left
    // the run reporting that the application was never reachable — which sends
    // someone to check the wrong system entirely.
    return finish({
      status: 'failed',
      locatorUsed: found.used,
      usedFallback: found.usedFallback,
      failure: classifyConfigurationError(
        monitor.snapshot(),
  ...
  // An assertion about "this step's element" has to be checked against the
  // locator that actually found it, not the one that was recorded.
  //
  // Without this, a step rescued by a fallback fails its own post-condition. The
  // fill lands on the right element via the fallback, then `inputFilled` is
  // evaluated against the stale primary locator, matches nothing, and reports
  // that the field was empty — the exact opposite of what happened. The verdict
  // was OUTCOME_NOT_MET, which is not healable, so a stale locator was reported
  // as an application failure and the run stopped. That is the single most
  // damaging misattribution this classifier can make, and it was silent.
  //
  // Only assertions pointing at the step's own element are rebound. One naming a
  // different element — the destination heading after a navigation — is left
  // exactly as recorded, because its identity is not this step's element.
  const outcome = rebindOutcomeToUsedLocator(step, found.used);

  const verification = await verifyOutcome(page, outcome);
  if (!verification.ok) {
    return finish({
      status: 'failed',
      locatorUsed: found.used,
...
/** Structural equality, so a rebind targets the step's element and nothing else. */
function sameLocator(a: Locator | null, b: Locator | null): boolean {
  if (!a || !b) return a === b;
  return (
...
/**
 * Points any assertion about the step's own element at the locator that found it.
 *
 * A fallback rescuing a step means the primary locator no longer matches. An
 * assertion still holding that primary locator would then be checked against
 * nothing and fail, which reads as the application misbehaving rather than as the
 * drift it is.
 */
export function rebindOutcomeToUsedLocator(
  step: BaselineStep,
  used: Locator | null,
): BaselineStep['expectedOutcome'] {
  if (!used || !step.locator || sameLocator(step.locator, used)) return step.expectedOutcome;

  return {
    ...step.expectedOutcome,
    assertions: step.expectedOutcome.assertions.map((assertion) =>
      sameLocator(assertion.locator, step.locator) ? { ...assertion, locator: used } : assertion,
    ),
  };
}
