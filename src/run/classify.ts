...
/**
 * The test could not be run as written: a value the plan refers to is not
 * available. Not the application's fault and not a stale locator, so it is
 * neither healable nor an outage.
 */
export function classifyConfigurationError(health: AppHealth, message: string): StepFailure {
  return failure(
    'CONFIGURATION_ERROR',
    `${message} The application was not at fault; the run was misconfigured.`,
    health,
  );
}
