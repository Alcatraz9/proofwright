...
import { pageSignatureSchema } from './visual.js';
...
export const baselineSchema = z.object({
  ...
  /**
   * What each page the run visits looked like, per viewport.
   *
   * Held against the page rather than the step: several steps happen on one page,
   * appearance is a property of the page, and one record per step would store the
   * same thing repeatedly and let the copies disagree.
   *
   * Defaulted so a baseline recorded before visual capture existed still loads.
   * Adding a required field to this schema previously made every stored artifact
   * silently unreadable, and the seed was skipped without a word.
   */
  visualBaselines: z.array(pageSignatureSchema).default([]),
});
