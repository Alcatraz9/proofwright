...
export const stepSchema = z.object({
  ...
  expectedValue: z
    .string()
    .nullable()
    // Defaulted rather than merely nullable so that adding a field to this
    // schema does not make every previously-stored plan unreadable. `default` is
    // stripped before the schema reaches the model, so generation still treats
    // this as a required nullable field and the grounding rule in validate.ts
    // still applies; an omitted value simply reads as "no literal asserted",
    // which is the same conservative meaning as an explicit null.
    .default(null)
    .describe(
