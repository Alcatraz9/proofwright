...
import { healSignature, readCachedProposal, writeCachedProposal } from '../store/heal-cache.js';
...
export interface ProposeHealResult {
  proposal: HealProposal;
  model: string;
  /** True when the proposal came from the cache and no model call was made. */
  fromCache: boolean;
  cacheHits: number;
}
...
export async function proposeHeal({
  ...
  snapshot,
  failureMessage,
}: ProposeHealParams): Promise<ProposeHealResult> {
  ...
  // Answer the same question twice and you get the same answer, at the cost of a
  // request against a daily quota. The cached value is a proposal, not a verdict:
  // it is still resolved to a locator, executed against the live page, and judged
  // by the step's original recorded outcome, so a hit cannot make a wrong heal
  // pass. It only skips the asking.
  const signature = healSignature({
    intent: step.intent,
    action: step.action,
    staleLocator,
    fingerprint,
    elementKeys: snapshot.elements.map((element) => `${element.role}|${element.accessibleName}|${element.testId ?? ''}`),
  });

  const cached = readCachedProposal(signature, snapshot.elements);
  if (cached) {
    return {
  ...
  const proposal = healProposalSchema.parse(data);
  writeCachedProposal(signature, proposal, model, snapshot.elements);

  return { proposal, model, fromCache: false, cacheHits: 0 };
}
