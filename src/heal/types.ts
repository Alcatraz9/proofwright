...
export const healAttemptSchema = z.object({
  ...
  /**
   * Whether the proposal was reused rather than requested.
   *
   * Surfaced rather than hidden: a reader is entitled to know whether the model
   * was consulted for this repair or whether an identical earlier answer was
   * reused. It changes nothing about correctness — the candidate was still
   * executed and still judged by the recorded outcome — but a cached proposal
   * presented as a fresh one would overstate what just happened.
   */
  proposalFromCache: z.boolean().default(false),
});
