...
async function main() {
  ...
  const { stored } = savePlan({ plan, model, targetUrl, instruction, planId });
  ...
  console.log(`Saved ${planId} as ${stored.status}.`);
  console.log(`Review the steps above, then: npm run approve -- --plan ${planId}`);
  console.log('Nothing can be recorded or replayed until the plan is approved.');
}
