...
async function main() {
  ...
  // The approval gate is enforced here rather than only in the API, so the
  // reviewed artifact is the one that gets recorded no matter which entry point
  // is used. A gate that one interface can walk around is not a gate.
  if (stored.status !== 'APPROVED') {
    throw new Error(
      `Plan "${planId}" is ${stored.status}. Review it, then: npm run approve -- --plan ${planId}`,
    );
  }

  const existing = loadBaseline(planId);
  if (existing && !force) {
