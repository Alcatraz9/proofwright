...
async function main() {
  ...
  const run = await replayBaseline({
    ...
    onHealableFailure: healing
      ...
      : undefined,
    onEvent: (event) => {
      // The CLI prints a transcript after each step resolves, so a "starting"
      // signal has nothing to add here. It exists for the live view.
      if (event.type === 'stepStarting') return;
      if (event.type === 'inspectionError') {
        console.log(`         inspection could not run: ${event.message}`);
        return;
      }
  ...
  saveRun(run, { healingEnabled: healing, healCount: healed.size });
  ...
  console.log(`Saved run ${run.runId}`);
